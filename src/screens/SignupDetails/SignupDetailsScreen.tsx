    ...
    <Screen>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: theme.surfaces.card, borderColor: theme.colors.cardBorder, shadowColor: theme.colors.teal900 }]}>
          ...
          <FormField label="Email (optional)" placeholder="Email" value={signup.email} onChangeText={text => setSignup(current => ({ ...current, email: text }))} icon="email" keyboardType="email-address" />
          ...
          <FormField
            ...
            onChangeText={text => setSignup(current => ({ ...current, country: text }))}
            icon="user"
          />
          ...
          />
          {signup.password.length > 0 && !isPasswordValid ? (
            <Text style={[styles.fieldErrorText, { color: theme.colors.giftAccent }]}>Password must be at least {MIN_PASSWORD_LENGTH} characters.</Text>
          ) : null}
          ...
          />
          {signup.confirmPassword.length > 0 && !passwordsMatch ? (
            <Text style={[styles.fieldErrorText, { color: theme.colors.giftAccent }]}>Passwords do not match.</Text>
          ) : null}
