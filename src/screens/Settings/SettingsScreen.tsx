...
function SectionLabel({ label }: { label: string }) {
  ...
  return <Text style={[styles.sectionLabel, { color: theme.text.secondary }]}>{label}</Text>;
}
...
function SettingsRow({
  ...
  return (
    <Pressable
      onPress={onPress}
      disabled={!onPress}
      style={[styles.row, { borderBottomColor: theme.colors.cardBorder }]}
    >
      <Text style={[styles.rowLabel, { color: destructive ? theme.colors.giftAccent : theme.text.primary }]}>{label}</Text>
      {rightElement ? (
        rightElement
      ) : value ? (
        <Text style={[styles.rowValue, { color: theme.text.secondary }]} numberOfLines={1}>
          {value}
        </Text>
      ) : onPress ? (
        <Text style={[styles.rowChevron, { color: theme.text.mutedIcon }]}>›</Text>
      ) : null}
    </Pressable>
  );
}
          ...
          <SettingsRow
            ...
            rightElement={
              <Switch
                ...
                trackColor={{ true: theme.colors.teal700, false: theme.colors.cardBorder }}
                thumbColor="#FFFFFF"
              />
            }
          />
